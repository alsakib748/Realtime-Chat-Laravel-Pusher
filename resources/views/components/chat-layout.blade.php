                ...
                <div class="p-4">
                    ...
                    @if ($generalRoom)
                        <div class="space-y-2 mb-4">
                            <button @click="loadRoom({{ $generalRoom->id }}, 'General Chat')"
                                ...
                                :class="currentRoom?.id == {{ $generalRoom->id }} ? 'bg-blue-50 border-blue-200' :
                                    'border-gray-200'">
                        ...
                        @foreach ($allUsers as $otherUser)
                            <div data-user-id="{{ $otherUser->id }}"
                                ...
                                <div class="relative flex-shrink-0">
                                    <img src="{{ $otherUser->getAvatarUrl() }}" alt="{{ $otherUser->name }}"
                                        class="w-8 h-8 rounded-full">
                                    <div
                                        class="status-dot absolute -bottom-1 -right-1 w-3 h-3 {{ $otherUser->isOnline() ? 'bg-green-400' : 'bg-gray-400' }} rounded-full border-2 border-white">
                                    </div>
                                </div>
                                <div class="flex-1 min-w-0">
        ...
        <div class="flex-1 flex flex-col bg-white min-w-0">
            <div class="p-4 border-b bg-gray-50 flex-shrink-0">
                <h2 class="text-lg font-semibold text-gray-800 truncate"
                    x-text="currentRoom ? currentRoom.name : 'Select a chat room'"></h2>
            </div>

            <div id="messages-container" class="flex-1 overflow-y-auto p-4 space-y-4" x-show="currentRoom"
                style="min-height: 0;">
                <template x-for="(message, index) in messages"
                    :key="'message_' + (message.uniqueKey || message.id || index) + '_' + index">
                    <div class="flex space-x-3" x-show="message && message.user">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mt-1">
                                <p class="text-gray-700 break-words" x-text="message.content" x-show="message.content">
                                </p>
                                ...
                                <template x-for="attachment in (message.attachments || [])">
                                    <div class="mt-2 max-w-sm">
                                        ...
                                        <div x-show="attachment.is_image" class="relative">
                                            ...
                                            <div x-show="attachment.uploading"
                                        ...
                                        <div x-show="!attachment.is_image"
                                            ...
                                            <div
                                                ...
                                                <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                                                    <path
                                                        d="M4 4a2 2 0 00-2 2v8a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2H4zm8 2a1 1 0 000 2h3a1 1 0 100-2h-3zM4 8a1 1 0 000 2h3a1 1 0 000-2H4z" />
                                                </svg>
                                            </div>
                                            <div class="flex-1 min-w-0">
                                                <h4 class="text-sm font-medium text-gray-800 truncate"
                                                    x-text="attachment.filename"></h4>
                                                <p class="text-xs text-gray-600" x-text="attachment.size"></p>
                                            </div>
                                            <div x-show="attachment.uploading" class="text-xs text-blue-600">
                                                Uploading...</div>
                                            <div x-show="!attachment.uploading" class="text-xs text-gray-500">Click to
                                                download</div>
                                        </div>
                                    </div>
                                </template>
            ...
            <div class="p-4 border-t bg-white flex-shrink-0" x-show="currentRoom">
                ...
                <div x-show="showFileModal"
                    ...
                    @click.self="clearSelectedFile()">
                    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                        <h3 class="text-lg font-semibold mb-4">Send File</h3>

                        <div x-show="filePreview" class="mb-4">
                            <img :src="filePreview" class="max-w-full h-auto rounded-lg"
                                style="max-height: 200px;">
                        </div>

                        <div x-show="selectedFile" class="mb-4">
                            <p class="text-sm text-gray-600"><strong>File:</strong> <span
                                    x-text="selectedFile?.name"></span></p>
                            <p class="text-sm text-gray-600"><strong>Size:</strong> <span
                                    x-text="selectedFile ? formatFileSize(selectedFile.size) : ''"></span></p>
                        </div>
                ...
                <div class="flex space-x-2 items-end">
                    ...
                    <input type="text" x-model="messageInput" @keyup.enter="sendMessage" @input="handleTyping"
                        @blur="stopTyping" placeholder="Type your message..."
